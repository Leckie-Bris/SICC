...
myrmidon_file1 <- paste("/media/ll16598/SeagateDesktopDrive/SICC_DATA/15P_IV50_250522_HDN/LLEXPP15P_IV50_250522_HDN_MAIN_KEY_DEATHS_AutoOriented.myrmidon",sep='')
Metadata_exp <- fmExperimentOpen(myrmidon_file1)

#FORPATHOGEN CHALLENGED COLONIES
CHALL_KVL_ <- paste(dir_chall, "15P_KVL.myrmidon",sep='') #Sham SI IDs
CHALL_KVL <- fmExperimentOpen(CHALL_KVL_)
CHALL_BB_ <- paste(dir_chall, "15P_BB.myrmidon",sep='') #Sham SI IDs
CHALL_BB <- fmExperimentOpen(CHALL_BB_)

#READ SI FILES
SIBB_ <- paste(dir_SI, "15P_SIBB.myrmidon",sep='') #Beauveria SI IDs
SIBB <- fmExperimentOpen(SIBB_)
SIKVL_ <- paste(dir_SI, "15P_SIKVL.myrmidon",sep='') #Metarhizium SI IDs
SIKVL <- fmExperimentOpen(SIKVL_)
SISH_ <- paste(dir_SI, "15P_SISH.myrmidon",sep='') #Sham SI IDs
