...
ref_orient_caps_file <-  EXP_list[which(grepl(EXP_list$path_name,pattern = "CapsuleDef4.myrmidon")),]
...
###ASSIGN METADATA KEYS IN META ANNOTATED EXP TO OTHERS###
# Select the metadata-rich files for the Queen ID check - same list here for me as queen metadata already added
#Metadata_list <- EXP_list[which(grepl(EXP_list$path_name,pattern = "withMetaData-base")),]
#Metadata_list <- ToOrient_file
#Metadata_file <- Metadata_list$path_name
#Metadata_list <- Metadata_list[which(!grepl(Metadata_list$path_name,pattern = "base")),]
#Metadata_exp <- fmExperimentOpen(Metadata_file) 
#Metadata_ants <- Metadata_exp$ants

####I do not need to do ### 
for (variable in ToOrient_file_path){
  AntsCreated  <- fmExperimentOpen(variable)
  dir_var <- dirname(variable)
  ...
  for (KEY in   1:length(Metadata_exp$metaDataKeys)) {
    ...
    AntsCreated_ants <- AntsCreated$ants ##SETS first ANT ID as QUEEN
  for (ant in AntsCreated_ants){
    individual  <- ant$ID
    #extract metadata info per key
    if (individual == "1"){
      ant$setValue("queen",TRUE, t)
        # }#skip if same
      }
  
  #save
  AntsCreated$save(paste0(sub("\\..*", "", variable),"_KEY.myrmidon"))  
}}
####END

#TO ONLY USE THE FILES WITH KEYS ASSIGNED
ToOrient_file <- EXP_list[which(grepl(EXP_list$path_name,pattern = "_KEY_DEATHS.myrmidon")),]
...
for (TS in unique(ref_orient_caps_file$space_name)){
  ...
  for (ToOrient_myr_file in ToOrient_data_list){
    ...
    if ( !file.exists(paste0(sub("\\..*", "", ToOrient_myr_file),"_AutoOriented.myrmidon"))) {
      ...
      to   <- fmTimeCreate(offset=fmQueryGetDataInformations(tracking_data)$start + 6*3600  ) ###experiment start time
      ...
      for (ant in ToOrient_ants){ #was metadata ants, but since I have Q IDs added, ToOrient_ants can be used
        individual  <- ant$ID
        IsQueen <- NULL
      ...
      #Metadata_ants[[QueenID]]$getValues("queen")
 ...
 #     REP.filefolder  <- paste(REP.folder,variable,sep="/")
   #   REP.dir <- dir_file(ToOrient_myr_file)
     # tracking_data$save(paste0(REP.dir),"_AutoOriented.myrmidon")
